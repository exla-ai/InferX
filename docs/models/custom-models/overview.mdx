...
optimized_model = optimize_model(
    model_path,
    precision="fp16",
    workspace_size=1 << 24,  # 16 MB workspace
    dynamic_shape=False,
...
- **Start with FP16 precision** for a good balance of speed and accuracy
- **Use INT8 quantization** when maximum performance is needed
- **Benchmark on your target hardware** to find the optimal configuration
- **Verify accuracy** after optimization, especially with INT8 quantization
- **Use batch processing** when possible for better throughput

With Exla, you can achieve dramatic performance improvements with minimal code changes, making it possible to run sophisticated models on resource-constrained devices.
