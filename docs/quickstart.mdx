...
Let's create a simple image search application using CLIP:
...
search_query = input("What kind of image are you looking for? ")

# Run inference
results = model.inference(
    image_paths=image_paths,
    text_queries=[search_query]
)

# Display top 3 results
print(f"\nTop matches for '{search_query}':")
for i, result in enumerate(results[0][search_query][:3]):
    print(f"{i+1}. {os.path.basename(result['image_path'])} (Score: {result['score']})")
```
