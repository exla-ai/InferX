...
class Clip_Jetson(Clip_Base):
    def __init__(self, model_name="openai/clip-vit-large-patch14-336"):
        ...
        self.cache_dir = Path.home() / ".cache" / "inferx" / "clip_trt"
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        
        # Print initialization message with animation
        with ProgressIndicator(f"Initializing InferX Optimized CLIP model for {device_type.upper()} [GPU Mode]") as progress:
